...
Deep learning models can predict stock prices well, but they don't explain the predictions they make. That matters in regulated contexts where traders, compliance teams, and regulators all need to understand how a prediction was reached. Tools like SHAP and WindowSHAP can answer that question after the fact, but they're expensive: Nori et al. (2023) report roughly 90 GB of RAM and tens of thousands of CPU-seconds per explained sample on datasets smaller than this one.

This thesis proposes **dynamic feature gating** as a built-in alternative. A small auxiliary network reads a summary of each input window and outputs a sigmoid gate value between 0 and 1 for each of the 21 input features. Those gate values scale the input before it enters the prediction model, and they also serve as real-time feature-importance scores, so no separate post-hoc step is required.
...
The dynamic gate produces a 21-dimensional feature importance vector for every test window. Averaged across all windows, the per-stock gate values show sector-consistent patterns:
...
On September 18, 2024, the Fed cut rates by 50 basis points, its first cut since March 2020. The gate's weighting of rate-related features that day is sector-consistent, **without the model having been told which stock is a bank**:

![JPM vs AAPL gate values on Fed rate cut date](results/figures/table_2_jpm_vs_aapl.png)

The gate assigned roughly 2× the weight to rate features for JPM (a bank) than for AAPL (a consumer tech firm). Non-rate features showed almost no difference between the two, so the gate isn't just assigning higher weights to JPM across the board. It's picking out which features matter for which stock.
...
Feature gating costs about 1 percentage point of accuracy vs. the vanilla TCN, in exchange for per-window feature attribution that comes free at inference time.
